function dbs_eeg_percept_direction_new_plot(initials, condition, method)
...
%     method='Granger'; %Coherence
    ...
    try
        files = spm_select('FPList','.', ['^' 'new_' '^.' initials '_rec_' num2str(1) '_' condition '_[0-9]*.mat']);
    catch
    ...
    for sub_condition=1:numel(D.condlist)
        ...
        files = spm_select('FPList','.', ['C_', condition, '_', subcondition, 'new_', '.', initials '_rec_' num2str(1) '_' condition '\w*.mat']);
        if isempty(files)
            files = spm_select('FPList','.', ['C_', condition, '_', subcondition, 'new_', initials '_rec_' num2str(1) '_' condition '\w*.mat']);
        end
        ...
        files = spm_select('FPList','.', ['C_', condition, '_', subcondition, 'new_', '.', initials '_rec_' num2str(2) '_' condition '\w*.mat']);
        if isempty(files)
            files = spm_select('FPList','.', ['C_', condition, '_', subcondition, 'new_', initials '_rec_' num2str(2) '_' condition '\w*.mat']);
        end
