...
if details.process_logfiles==1 && ~isempty(files{3})
    ...
    logfile=dbs_eeg_percept_logfiles_prepare(S.dataset, files{3});
else
...
if details.process_videos==1 && ~isempty(files)
    ...
    video_file=dbs_eeg_percept_videofiles_prepare(S.dataset, fullfile(files{4}, '\'), spm_file(files{4}, 'filename'));
