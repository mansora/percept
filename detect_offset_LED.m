function [offset_start, offset_end, n2]=detect_offset_LED(filename_LED_video, input_logfile)
...
if ~isempty(t)
    ...
    disp('no initializing LED sequence found')
end
...
for f=1:videoIn.NumFrames
    ...
    for i=1:n_sampling
        LED_condition(i,f,:)=squeeze(videoFrame(y(i), x(i),:));
    end
...
% TODO now the sampling rate of EEG is always assumed to be a 1000 but you
% probably want to add it as an input to the function to avoid errors in
% the future when EEG sampling rate is not necessarily 1000
cfg=[];
cfg.resamplefs= 1000;
